...
@section('content')
            ...
            <div class="bg-white rounded-2xl shadow-sm overflow-hidden mb-8">
                ...
                <div class="relative">
                            ...
                            <div id="image-carousel" class="flex transition-transform duration-500 ease-in-out">
                                @foreach ($foodPlace->businessImages as $img)
                                    <div class="flex-shrink-0 w-full">
                                        <img src="{{ $img->image_url }}" alt="{{ $foodPlace->title }}"
                            ...
                            @if ($foodPlace->businessImages->count() > 1)
                                ...
                                <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2">
                                    @foreach ($foodPlace->businessImages as $index => $img)
                                        <button
                                            class="carousel-indicator w-2 h-2 rounded-full bg-white/60 hover:bg-white transition-all duration-300 {{ $index === 0 ? 'bg-white' : '' }}"
                    ...
                    <div class="absolute top-4 left-4 right-4 flex justify-between items-start">
                        ...
                        <div
                            class="bg-white/95 backdrop-blur-sm px-3 py-1.5 rounded-full shadow-lg flex items-center space-x-1">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                @foreach ($foodPlace->menuImages as $menuImage)
                                    <div class="relative group cursor-pointer"
                                        ...
                                        <img src="{{ $menuImage->image_url }}" alt="Menu {{ $foodPlace->title }}"
            ...
            @if (isset($foodPlace->source_location))
                <div class="bg-white rounded-2xl shadow-sm overflow-hidden mb-8">
                    <div class="p-6 md:p-8">
                        ...
                        <!-- Map Placeholder -->
                        <div
                            class="bg-gradient-to-br from-blue-50 to-green-50 rounded-lg h-64 mb-4 flex items-center justify-center border border-gray-200">
                            <div class="text-center">
                                <svg class="w-12 h-12 mx-auto text-orange-500 mb-3" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M17.657 16.657L13.414 20.9a2 2 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            @if ($review->taste_rating)
                                                <div class="flex items-center space-x-2">
                                                    ...
                                                    <div class="flex space-x-1">
                                                        @for ($i = 1; $i <= 5; $i++)
                                                            <div
                                                                class="w-2 h-2 rounded-full {{ $i <= $review->taste_rating ? 'bg-yellow-400' : 'bg-gray-300' }}">
                                                            </div>
                                                        @endfor
                                                    </div>
                                                </div>
                                            ...
                                            @if ($review->price_rating)
                                                <div class="flex items-center space-x-2">
                                                    ...
                                                    <div class="flex space-x-1">
                                                        @for ($i = 1; $i <= 5; $i++)
                                                            <div
                                                                class="w-2 h-2 rounded-full {{ $i <= $review->price_rating ? 'bg-green-400' : 'bg-gray-300' }}">
                                                            </div>
                                                        @endfor
                                                    </div>
                                                </div>
                                            ...
                                            @if ($review->service_rating)
                                                <div class="flex items-center space-x-2">
                                                    ...
                                                    <div class="flex space-x-1">
                                                        @for ($i = 1; $i <= 5; $i++)
                                                            <div
                                                                class="w-2 h-2 rounded-full {{ $i <= $review->service_rating ? 'bg-blue-400' : 'bg-gray-300' }}">
                                                            </div>
                                                        @endfor
                                                    </div>
                                                </div>
                                            ...
                                            @if ($review->ambiance_rating)
                                                <div class="flex items-center space-x-2">
                                                    ...
                                                    <div class="flex space-x-1">
                                                        @for ($i = 1; $i <= 5; $i++)
                                                            <div
                                                                class="w-2 h-2 rounded-full {{ $i <= $review->ambiance_rating ? 'bg-purple-400' : 'bg-gray-300' }}">
                                                            </div>
                                                        @endfor
                                                    </div>
                                                </div>
                                ...
                                @if ($review->tags && count($review->tags) > 0)
                                    <div class="flex flex-wrap gap-2 mb-4">
                                        @foreach ($review->tags as $tag)
                                            <span
                                                class="inline-block px-3 py-1 text-xs font-medium text-orange-700 bg-orange-100 rounded-full">
                                                #{{ $tag }}
                                            </span>
                                        @endforeach
                                    </div>
    ...
    <!-- Image Modal -->
    <div id="imageModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/75"
