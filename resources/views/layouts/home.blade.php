   ...
   <section class="py-8 bg-white relative z-10">
    <div class="container mx-auto px-4">
        <div class="max-w-4xl mx-auto bg-white rounded-xl shadow-xl p-6 -mt-20 relative z-20 animate-float-up">
            <form action="{{ route('food.search') }}" method="GET" class="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($foodPlaces as $foodPlace)
                    <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                        @if($foodPlace->image)
                            <img src="{{ asset('storage/' . $foodPlace->image) }}" alt="{{ $foodPlace->name }}" class="w-full h-48 object-cover">
                        @else
                        ...
                        <div class="p-4">
                            ...
                            @if($foodPlace->category)
                                <span class="inline-block bg-orange-100 text-orange-800 text-xs px-2 py-1 rounded-full mb-2">
                                    {{ $foodPlace->category->name }}
                                </span>
                            @endif

                            @if($foodPlace->description)
                                <p class="text-gray-600 text-sm mb-2">{{ Str::limit($foodPlace->description, 100) }}</p>
                            @endif
                            ...
                            <div class="mt-3">
                                <a href="{{ route('food-place.show', $foodPlace->id) }}" class="text-orange-500 hover:text-orange-600 font-medium text-sm">
                                    Lihat Detail →
                                </a>
                            </div>
                        </div>
